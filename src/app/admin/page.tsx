...
function AdminContent() {
  ...
  async function removeUser(p: ProfileRow) {
    ...
    // Remove from the visible list for property admins; corporate still sees deactivated
    if (currentUser && isPropertyAdmin(currentUser)) {
      setProfiles((prev) => prev.filter((x) => x.id !== p.id));
    } else {
      setProfiles((prev) =>
        prev.map((x) =>
          x.id === p.id ? { ...x, is_active: false, hotel_id: null } : x
        )
      );
    }

    setMessage(`${label} has been removed and can no longer sign in.`);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                  ...
                  profiles.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-right whitespace-nowrap">
                        ...
                        {p.id !== currentUser?.id && !p.is_active && isCorp && (
                          <button
                            type="button"
                            disabled={savingId === p.id}
                            onClick={() =>
                              updateProfile(p.id, { is_active: true })
                            }
        ...
        <div className="mt-4 text-xs text-gray-400 space-y-1">
          <p>
            <strong>Remove</strong> stops the person from signing in (use when
            they leave the company).
          </p>
          {isCorp && (
            <p>
              Corporate Admins can <strong>Restore</strong> a removed user if
              needed.
            </p>
          )}
        </div>
